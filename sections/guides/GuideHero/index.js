...
/**
 * Guide page hero (h1 + lede + optional Buy). Reads `hero` from the guide namespace.
 * @param {{ namespace: string }} props
 */
export default function GuideHero({ namespace }) {
  const { t } = useTranslation(namespace);
  const hero = t("hero", { returnObjects: true });
  const buy = hero?.buy;

  return (
    <Container className="section-guide-hero" align="center" halign="center">
