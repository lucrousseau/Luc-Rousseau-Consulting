...
type CvVariant = "engineer" | "pm" | "dev" | "react" | "techlead" | "founding";
 ...
 */
export default function Cv({ variant = "engineer" }: CvProps) {
  const pos = CV_POSITIONING_NS.get(variant) ?? "cv";
  const { t } = useTranslation(["cv", pos, "common"]);

  const highlights = t(`${pos}:highlights.items`, { returnObjects: true }) as {
    value: string;
    label: string;
  }[];
  const strengths = t(`${pos}:strengths.items`, { returnObjects: true }) as {
    title: string;
    content: string;
  }[];
  const experience = t(`${pos}:experience.items`, { returnObjects: true }) as {
    role: string;
    company: string;
    companyHref?: string;
    period: string;
  ...
  const skillGroups = t(`${pos}:skills.groups`, { returnObjects: true }) as {
    title: string;
    items: string[];
  }[];

  const contactItems = t("cv:contact.items", { returnObjects: true }) as {
    label: string;
    href: string;
    value: string;
    note?: string;
  }[];
  const earlier = t("cv:earlier.items", { returnObjects: true }) as {
    role: string;
    company: string;
    period: string;
  }[];
  const languages = t("cv:languages.items", { returnObjects: true }) as {
    name: string;
    level: string;
  }[];
  const awards = t("cv:awards.items", { returnObjects: true }) as string[];
