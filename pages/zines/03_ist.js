...
export default function Zine({ images }) {
  const { t } = useTranslation(["common", "zines", "zine-03-ist"]);

  return (
    <>
      <SEO
        title={t("zine-03-ist:zine-seo-title")}
        description={t("zine-03-ist:zine-seo-description")}
        ...
        <Container tag={"carousel"}>
          <Row
            ...
            columns={[
              {
                cols: { col: 4, lg: 12 },
                content: parse(t("zine-03-ist:content")),
              },
              {
                cols: { col: 8, lg: 12 },
...
export const getServerSideProps = async ({ locale }) => {
  const folder = "images/zines/03_ist";
