...
export default function Zine({ images }) {
  const { t } = useTranslation(["common", "zines", "zine-04-cai"]);

  return (
    <>
      <SEO
        title={t("zine-04-cai:zine-seo-title")}
        description={t("zine-04-cai:zine-seo-description")}
        ...
        <Container tag={"carousel"}>
          <Row
            ...
            columns={[
              {
                cols: { col: 4, lg: 12 },
                content: parse(t("zine-04-cai:content")),
              },
              {
                cols: { col: 8, lg: 12 },
...
export const getServerSideProps = async ({ locale }) => {
  const folder = "images/zines/04_cai";
