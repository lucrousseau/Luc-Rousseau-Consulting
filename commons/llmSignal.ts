...
export function getLlmBoundaries(): string[] {
  return [
...
function buildHomeSectionsBlock(base: string): string {
...
export function buildExpertiseSection(base: string): string {
...
export function buildSituationsSection(base: string): string {
...
function buildBoundariesBlock(): string {
...
export function buildHumansTxt(base: string): string {
  const { email, calendly, linkedin } = LLM_CONTACT;
  const frHomeSeo = getHomeSeoCopy("fr");
  const enHomeSeo = getHomeSeoCopy("en");
